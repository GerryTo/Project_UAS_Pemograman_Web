...
$id= $_GET['id'];
$data = $database->show_id_talks($id);
$talks_data = mysqli_fetch_array($data);
...
if(isset($_POST['create']))
{
    ...
    if($database->update_talks($id,$title,$author,$date,$isi,$name,$target_file)){
    ...
    <div class="admin-content">
        ...
        <div class="content">
            ...
            <form action="" method="post" enctype='multipart/form-data'>
                <div>
                    <label>Title</label>
                    <input type="text" name="title" value="<?php echo $talks_data['title']?>"class="text-input">
                </div>
                <div>
                    <label>Author</label>
                    <select name="author" class="text-input" required>
                    <?php
                        $author_talks = $talks_data['id_author'];
                        while($author = mysqli_fetch_array($result)){
                            $ket="";
                            if(isset($_GET['author'])){
                                if($author_talks==$author['id']){
                                    $ket="selected";
                                }
                            }
                      ?>
                      <option <?php echo $ket; ?> value="<?php echo $author['id'];?>"><?php echo $author['username'];?></option>
                      <?php
                      }
                      ?>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="currentDateTime" name="date" value="<?php echo $talks_data['date']?>"class="text-input">
                </div>
                <div>
                    <label>Content</label>
                    <textarea name="isi"id="content"><?php echo $talks_data['isi']?></textarea>
                </div>
                <div >
                <video src="<?php echo $video?>" controls width='320px' height='200px' >;
                </div> 
                <div>
                    <label>Video</label>
                    <input type="file" name="file" class="text-input" value="<?php echo $talks_data['name']?>">
                </div>
                <div>
                    <button type="submit" name="create" class="btn btn-size">Edit Post</button>
                </div>               
            </form>
        </div>
    </div>
